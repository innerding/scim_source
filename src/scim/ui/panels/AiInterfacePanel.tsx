...
const SEED_ANNOTATIONS: Annotation[] = [
...

  {
    ...
    content: 'STERNENSCHEIBE-KOSMOLOGIE · BAUABSCHNITT 2 „Neu entwickeln" (Operator 2026-06-13). Folgt auf BA1 (ann_144). Hier entsteht ECHTE neue Funktion. — GATE-PRINZIP (Operator 2026-06-13): Der GROSSE BÄR liegt UPSTREAM und ist das zentrale Import-/Cleaner-Gate — nichts kommt roh rein, weder Icons für Draco noch Glyphen für den Kleinen Bären. Alles läuft erst durch den Cleaner, dann in einen Speicher. EIN Gate, EINE Engine, ZWEI Profile (Icon: fill/stroke·48/24·Node-Budget · Glyph: Mittellinie·Box130·advance). Das in BA1 gebaute glyphImport (Kleiner Bär) ist schon das Glyph-Profil und konvergiert in BA2 hierher. User-/Regio-Dashboard nutzt möglichst DIESELBE geteilte Cleaner-Quelle = ein eigenes AUTHORING-Modul, NICHT shell-kit (shell-kit gehört zur Ziel-App, nicht zum Regio-Dashboard); wo die geteilte Quelle wohnt = OFFEN. Invariante: nichts landet un-gecleant in einer Representation. KONSEQUENZ: BA2 beginnt mit dem Großen Bären (alles hängt davon ab). — (1) GROSSER BÄR = Icon-Schmiede: der eigentliche Node/Icon-Editor nach der IconBuild-Spec (heute nur Baukonzeptnotiz im Drawer): Schwarz-Weiß-Stroke/Fill-Layer, Raster+Snap, variable Stroke-Breiten, Stroke→Fill, Boolean, Node-Budget (60, DP-Vereinfachung), Import (Raster-Pausschicht + .svg mit Node-Zahl), Cleaner→Konvention, Export+Provenienz, 3 Speicher-Ziele (data/icons · data/icons-shell · data/icons-scim). = der „Illustrator-Klon"; aus ihm geht auch die Schrift hervor (Glyphen wandern in den Kleinen Bär). Gemeinsamer Stroke-Asset-Kern mit der Font-Schmiede. — 7-TAB-PLAN (Operator 2026-06-13, beschlossen): Tabs nach den 7 Wagen-Sternen. t1·Dubhe = Intro/Access/Import: zeichnet den Großen Wagen GROSS IM PANEL-CANVAS als Legende/Inhaltsverzeichnis (jeder Stern beschriftet mit Name + Tab-Funktion, KLICKBAR → springt in seinen Tab; NICHT im Himmel/NavConstellations!) + Sofort-Import-Box. t2·Merak = Cleaner/Routing/Schutt: Cleaner läuft, Ziel-Zuordnung (Draco·data/icons / Cepheus·data/icons-scim / Kleiner-Bär·Glyph) + Speicherverwaltung, bestehendes Set aufrufen→Zeichen/Icon tauschen/ergänzen, Schutt-Container für ersetzte Alt-Assets (wie Müllwagen). t3·Phecda = CANVAS (großes Zeichenpanel, 48/24, Raster, Pen/Select; Clutter ausgelagert). t4·Megrez = Layer/Struktur (fill/stroke sperrbar, Layer-Gruppen-Name). t5·Alioth = Werkzeuge/Operationen (variable Stroke-Breiten 0.20er, Stroke→Fill, Boolean, Raster/Snap-Config). t6·Mizar = Vorschau/Kontext (Inspector-Kopplung: Icon im Container-Umfeld + Animationen; Mizar=Doppelstern mit Alcor). t7·Alkaid = Prüfung/Export/Provenienz (Node-Budget-Check muss bestehen + DP-Vereinfachung; Ersteller in <metadata> der Quell-SVG, vom Build-Cleaner fürs ausgelieferte Glyph WIEDER ABGESTREIFT = winzig; Download/Commit in den in t2 gewählten Speicher). BAU-REIHENFOLGE („erst ohne Canvas"): zuerst die traktable Spine t1(Legende+Import)·t2(Cleaner/Routing/Schutt)·t7(Check/Export/Provenienz); Mal-Canvas t3 + t4/t5/t6 als spätere Canvas-Unterphase. Canvas-Boolean/Pfad-Ops: Library vs Eigenbau noch offen. — (2) DRACO = Geo-Katalog: echter Nation→Region→Representation-Baum mit VERERBUNG (Sicht-Vererbung als Fundament: ein Icon liegt auf einer Ebene, tiefere sehen es mit; Override als Kür: Rep darf geerbtes Icon überschreiben — CSS-Kaskade-artig). „In der Nation ist alles", Region/Rep spezialisieren. Browse + Verwaltung + Routing in die 3 Speicher. Representationen leben im Baum, werden NICHT als Sterne gezeichnet. — (3) FONT-SCHMIEDE-FEINSCHLIFF (eigene, schon geplante Spur): Metriken-Tab entrümpeln (Skelett global, Set-once); neue „Satz-Werkstatt" (pro Glyph: Seitenbänder lsb/rsb via getBBox + Letter-Stretch scaleX); „Specimen" (pro Stroke-Set = Gewicht: GAP-STRETCH-Faktor auf die Bänder, alle Sets sichtbar). Knoten-Editor NICHT hier (→ Großer Bär). — (4) BACKEND-FRAGE (zu klären): Trennung scim_source ↔ sensus-core-runtime — wo liegen und wie reisen die GLOBALEN Assets (Schrift + Icons sind global, nicht per-Rep): via shell-kit ins Runtime, nicht im Per-Rep-Bundle. Cassiopeia/Cepheus/Draco lesen aus scim_source/data; Auslieferung global. Bezug: project_schriftenpanel.md, ann_144 (BA1), IconBuildNotiz (DrawerPanel.tsx).',
...

  {
    id: 'ann_138',
    category: 'next_intent',
    label: 'ARCHITEKTUR — Modal/Director-Console (Path/Rest-Symmetrie) + Umbauplan; Regel: kein Reroute ohne Commit',
    content: 'ARCHITEKTUR + UMBAUPLAN (Operator 2026-06-12, ausgeruht). NEUE HARTE REGEL: KEINE UMLEITUNG OHNE USER-COMMIT — die Route ist STABIL, ändert sich nie still (verschärft ann_136: kein „klein=auto" mehr). — DAS BILD = perfekte PATH/REST-SYMMETRIE (ann_127): jede der zwei Sorgen hat ein MINI-MODAL (Info/Eingang, für ungeübte User) + eine DIRECTOR-CONTROL (die Entscheidung). REST (POI) existiert: Mini-Modal „Besucherdruck" ignore/change (4c-ii) + Director-Control POI löschen→Alternativen/intake (4c-iii). PATH (Abschnitt/Umweg) NEU, das Geschwister: Mini-Modal „Umweg möglich" ignore/change + Director-Control Abschnitt löschen→Umwege wählen. — SCHLÜSSEL-EINSICHT: beide Director-Controls haben DIESELBE GESTALT — [Problem LINKS, zappelt] → [Lösungen RECHTS, durchklicken, je Zeit + Comfort]. Also NICHT 2 getrennte Komponenten, sondern EINE Director-Control-Gestalt mit zwei Inhalten (POI/Path); ebenso EINE Mini-Modal-Gestalt, zwei Inhalte. Die „2+2" = 2 Schalen × 2 Inhalte; die Path-Schalen ERBEN von den stehenden POI-Schalen. „Abschnitt löschen" = das belebte Segment meiden → triggert Umweg-Optionen (mirror: POI löschen → Alternativen). — DIE KETTE: (1) ein Route-Segment reißt Comfort. (2) Gibt es einen Umweg, dessen Comfort der EINSTELLUNG entspricht? JA → Path-Flow (Info-Modal-Path → Director-Controls-Path: Umwege mit Zeit + Comfort) → Commit. NEIN → Rest-Flow (Info-Modal-POI → Director-Controls-POI: löschen/tauschen) → Commit. (3) NICHTS ohne Commit. Der Deckel entscheidet jetzt nur WELCHE Control (Umweg vs POI), nicht auto-vs-commit. — COMFORT-STUFEN je Umweg: sehr ruhig · ruhig · moderat · frequentiert · belebt · voll = die 6 Stufen → aus stageOf (Spitzenlast des Umwegs → Stufe → Label). Kein Umweg wird angeboten, dessen Comfort unter der Einstellung liegt (dann Rest-Flow). — STÜTZRÄDER: die Mini-Modale sind für ungeübte User; sind Director-Controls + Play-Blinken vertraut → abschaltbar (Novice/Expert-Modus). — WAS ES MEHR BRAUCHT (über die 2+2): 🔴 PATH-DETOUR-GENERATOR (mehrere Umwege + Zeit + Comfort-Stufe; Geschwister von dompteurPicks, für Pfade — der neue Kern) · 🟡 Resolver-Kette (komfortabler Umweg→Path-Control, sonst→POI-Control; nichts auto = W4 realisiert) · 🟡 intent auf Path erweitern (Detour-Wahl in committed⊕intent→preview) · 🟡 Footer = EIN Director-Console (beide Gestalten + Blink) · 🟢 Comfort-Label (6 via stageOf) · Icons (off/intake hat Operator + Path-Geschwister) · Novice/Expert-Toggle · Zeiten. FUNDAMENT: das ist im Kern W4 (deliberate routing) + lehnt an W1/stageOf. Diese UX IST die sichtbare Form von W4. — UMBAUPLAN (gestaffelt): A·FUNDAMENT (W4): Detour-Generator + Resolver-Kette (Path→POI) + „kein Reroute ohne Commit" — ohne A hat die Path-Control nichts zu zeigen; erster mock-unabhängiger Stein = Path-Detour-Generator als reine shell-kit-Funktion. B·INTENT/PREVIEW für Path (Reuse der Maschine). C·UI: Director-Control-Gestalt generisch (POI+Path) + Mini-Modal generisch + Footer als Director-Console. D·POLITUR: Comfort-Labels, Icons, gestapelte Umweg-Darstellung (ann_131), Novice/Expert, Zeiten. — DANACH (Operator): „schauen, was wir an Modalen haben und brauchen, und den Footer mit den Director-Controls ausgestalten." Bezug: ann_127 (Path/Rest), ann_136 (Commit-Bindung), ann_130 (W4/Routing), ann_128 (stageOf/Comfort), ann_131 (Stapel), ann_126 (4c-Maschine), ann_137 (Backlog).',
    related_panel: 'R07',
    date: '2026-06-12',
  },

  // ── OFFENE PUNKTE — konsolidierter Backlog (Einstieg) (2026-06-11) ──

  {
    id: 'ann_137',
    category: 'next_intent',
    label: 'OFFENE PUNKTE — konsolidierter Backlog (Einstieg-Liste, Stand 2026-06-11)',
    content: 'KONSOLIDIERTER BACKLOG (2026-06-11) — verlässlicher Einstieg. Größe: 🔴 groß · 🟡 mittel · 🟢 klein. — FERTIG (Fundament): Route entkoppelt (neutraler Pfad, v0.55) · Netz über Comfort = ruhiges helles Grau (v0.57) · Route-Engpass-Segmente zappeln (Stroke-Puls, v0.58) · Weg-Nummern in der POI-Ecke (handgeschrieben, v0.59) · absolute Last + stageOf + Coloursample (v0.51/0.52, Step 0/1) · POI-Gestalt poiVisualState 4a/4b · 4c-Entscheidungs-Maschine komplett (Mini-Modal-Eintritt + IGNORE-Episode + Drop-up single 4c-iii-a & multi 4c-iii-b, committed⊕intent→preview, Play-commit, Vorschau-Rahmen). — — DIE 4 GROSSEN WURZELN (alles andere sitzt darüber): (W1) 🔴 LASTBERECHNUNG AUS DICHTE — heute ist die Last ein SYNTHETISCHES Feld (fieldAt), keine echte Dichte. Offen: (a) synthetisch → echte Dichte/Telco, (b) Abstands-Leiter KALIBRIEREN (250/150/50 m → die borders, die noch evenBorders sind), (c) PATH-lineare Einheit (Pers./Meter). Die Rest-areale Einheit steckt in W3. (ann_128, der lange Last-Modell-Tag.) — (W2) 🔴 HYSTERESE / stateful BCK — der Comfort-Schnitt ist ein nacktes `>` (kein Deadband, flackert); shell-kit muss ZUSTAND bekommen; Naht poiBreaches liegt bereit. (ann_123/128.) — (W3) 🔴 BCK-REST / RADIEN — Radien um POIs zerschneiden das Mesh in Rast(Rest)/Gehen(Path); eigenes (areales) Berechnungsmodell. (ann_127/128.) — (W4) 🔴 STUFE-1 DECKELN + UMWEG-COMMIT — das Routing-Keystone: Auto-Umleitung wird committete Wahl; Deckel = Auto/Commit-Grenze (klein→auto; groß/kein-Comfort→zappel+dashed+Play-commit). Erst dadurch wird das Zappeln sinnvoll. (ann_130/136.) — — ROUTING-DARSTELLUNG: 🟡 dashed Umwege (Resolver zeigt Route UND Umweg gleichzeitig, ann_135, hängt an W4) · 🟡 „Comfort-Button funktioniert anders" (heute Arbeits-Werkzeug; realer User setzt Comfort einmal, Last variiert; Konzept offen, ann_136). — COMFORT-SKALA: 🟡 6-Stufen scharf stellen (im Editor 6 stops + Grenzen + Lichtenberg neu publishen; nur Farb-Config, NICHT W1). — DIRECTOR-CONTROLS-POLITUR (4c-iii-c): 🟡 POI im Mini-Modal zeigen (heute nur Text; soll das betroffene POI flippend zeigen, Operator-Befund) · 🟡 per-POI „behalten" (ging in 4c-iii-b verloren, nur noch via Session-IGNORE) · 🟡 gestapelte/versetzte Alternativen mit Tap-to-cycle + Zeit + Uhr live (ann_131, Welk/Blüh GESTRICHEN) · 🟡 Tap auf Alternativ-POI → Modal am POI auf der Karte (ann_135, braucht POI-Info-Modal ↓) · 🟢 7-Spalten-Footer-Raster + INTAKE/OUT-Labels (Tabler-Icons). — POI-INFO-MODAL + DATEN (MVP-kritisch): 🟡 Long-tap-POI-Detail-Modal + S4-Ankunfts-Card (teilen die Komponente) · 🟡 Origin-POI-Daten: Kurzbeschreibung/Foto/Link ins Origin-POI-Set (heute nur Name+Subkat) — Datenabhängigkeit. — KLEINKRAM: 🟢 tote Dateien PoiManege.tsx + PoiDompteurEnergy.tsx (ungenutzt, Choreografien als Assets geparkt) können weg · 🟢 Stellschrauben (Zappel-Tempo Route 0.6s, Badge-Inset px/3-13, Netz-Grau #c2c9d2) auf Zuruf · 🟢 Alternativen-Filter lockern („relativ ruhiger" statt „voll im Comfort", ann_135, bewusster Tweak) · ⚪ Welk/Blüh auf UNBESTIMMTE Zeit verschoben (Asset geparkt). — ABNAHME: 🟡 end-to-end live auf gegateter diesenpark.com (echtes Gerät, QR) · 🟢 GPS/S5 = post-MVP. — ABHÄNGIGKEITSKETTE: W4 (Stufe-1 deckeln+Commit) ist der Schlüssel — danach ergeben Zappeln/dashed/Director-Controls voll Sinn. W1 (Dichte→Last) + W2 (Hysterese) sind die parallelen Last-Wurzeln. POI-Modal+Daten = MVP-kritischer Rest. Rest = Politur. Bezug: ann_126–136.',
    related_panel: 'R07',
    date: '2026-06-11',
  },
...

  {
    id: 'ann_128',
    category: 'invariant',
    label: 'LAST→STUFE — absolute Verankerung (kein Perzentil!) · zwei Einheiten · Hysterese',
    content: 'KERN-ENTSCHEIDUNG zum Last-Modell (Operator+Claude 2026-06-11) — klärt die offene absolut-vs-relativ-Frage aus ann_123/124. — CODE-BEFUND (shell-kit anthem.ts): normalizeLoads hat `spread` (0=roh/ABSOLUT … 1=voll RELATIV: min→kalt/max→heiß) + `floor` (Mindest-Rot, hebt den Peak ZWANGSWEISE auf rot). → die gefürchtete Pathologie „fast leeres Netz wird als total belebt ausgegeben" ist ein TUNBARER KNOPF; spread>0/floor>0 ERZEUGT sie genau. `fieldAt` = synthetisches 0..1-Feld, NOCH KEINE echte Dichte. Runtime-Schnitt heute `dimmed = average > comfort` = nacktes `>`, zustandslos. — DREI GETRENNTE BEGRIFFE (Operator nannte alle drei „Hysterese" → daher die Verknotung): ① SKALA = loadToStage(last)→1..6 (Kalibrierung, „die Tabelle") · ② ABSOLUT-vs-RELATIV = der Bezugspunkt der Grenzen (= der spread-Knopf) · ③ HYSTERESE/Deadband = temporales Anti-Flacker an EINER Schwelle (orthogonal zu ①②). Operator-Frage „ist das Ineinander-Übergehen der zwei Berechnungsansätze die Hysterese?" → NEIN, das ist ①/②, NICHT ③. — ② ENTSCHEIDUNG = ABSOLUTE VERANKERUNG BEIDER ENDEN (Operator SELBST hergeleitet: „die Maximal-Last ist nur dann die max BCK-Last/Stufe 6, wenn sie 1 P/m² detektiert; sonst existiert Stufe 6 im Mesh NICHT"): Stufe 0 = 0 Personen (leer=leer) · Stufe 6 = 1 P/m² (physische Sättigung). Beide Enden festgenagelt → ein fast leeres Netz landet überall bei Stufe 1–2 und zeigt KORREKT grün; Stufe 6 ist kein „das Vollste was gerade da ist", sondern ein FIXER Punkt, der an einem ruhigen Tag SCHLICHT NICHT erreicht wird. → das PERZENTIL-Modell aus ann_123 (Claude-Vorschlag) hat GENAU diesen Fehler → für die Last-MESSUNG VERWORFEN. RELATIV bleibt NUR für die PRÄFERENZ-Abbildung (User-Comfort 1–6 = „wieviel absolute Last toleriere ich"), NICHT fürs Messen. Deckt sich mit der Telco-Klärung in ann_123 (Auslastung absolut/gedeckelt; „relational" nur die Präferenz). MVP: spread=0, floor=0. — UMGEKEHRTE SORGE (Nicht-Ruhesucher bekommt nur winzige Auswahl) löst sich unter absoluter Verankerung AUTOMATISCH: ist das Netz echt ruhig, hat JEDER (auch Comfort 1) das ganze Netz, weil nichts die Schwelle reißt; Einschränkung NUR bei echter Last, proportional zur echten Fülle, nicht vom Ranking erfunden. EINZIGER Rest: wenn ALLES voll → Comfort-1-Mensch ohne Ziel → DA (nur da) die „in die Wüste"-Mechanik als RELATIVER NOTNAGEL („gib das relativ Ruhigste"). Randfall, POST-MVP. — ① TABELLE = ZWEI TABELLEN (trifft ann_127): die Operator-Zeilen mischen „Personen je Segment" (Strecke) + „Personen/4m² im Radius" (POI) — das sind die ZWEI KERNEL mit ihren NATÜRLICHEN EINHEITEN: BCK-PATH = LINEARE Dichte (Pers./Meter Strecke, Stau, 1-dim) · BCK-REST = FLÄCHEN-Dichte (Pers./m² im Radius, Rast, 2-dim). Jeder Kernel verankert seine EIGENE absolute Skala 0→Sättigung, je 6 Stufen. „Unten Headcount, oben Dichte" ist KEIN Ansatz-Wechsel, sondern EINE Dichtefunktion, die bei winzigen Zahlen wie Kopfzählen aussieht (1 Pers./200 m = praktisch leer). Exakte Zahlen (2/3 Pers., 4 m²…) = Kalibrierung (später, evtl. je Rep); JETZT festnageln = die STRUKTUR (absolut verankert, zwei Einheiten, monoton steigend). — ③ HYSTERESE = Eintritts- ≠ Austritts-Schwelle, Deadband = der Spalt dazwischen. WARUM: Comfort 3, stetige Live-Last pendelt 2,9/3,1/2,9/3,1 (tut ein stetiges Signal immer) → `>` kippt jeden Tick → Farbe strobt + Bein erscheint/verschwindet als breaching → ROUTE rechnet neu = der On/Off-Switcher. MIT Hysterese: Eintritt in breaching erst >3,75, Austritt erst <3,25, dazwischen bleibt es (Thermostat 19,5°/20,5°). FEHLT: überall wo der Comfort-Schnitt heute ein nacktes `>` ist — (a) dimmed/Mesh-Farbe, (b) breaching-Bein-Erkennung der Kaskade. Braucht GEDÄCHTNIS JE SEGMENT (STATEFUL: vorigen Zustand über die Ticks tragen) — Kernel heute zustandslos (Last rein→dimmed raus) = die eigentliche neue Arbeit; gleiche „Episode"-Idee wie der Ignore (ann_126). 6-vs-12-KREIS: die 0,5-Schritte der Operator-Tabelle (=12) SIND die Deadband-Leitplanken um die 6 sichtbaren Grenzen (hochstufen am oberen Halbschritt, runter am unteren) = ann_123 „intern 12 gegen Flackern". Operator hat die Deadband-Struktur hingeschrieben, ohne sie zu benennen. — VEREINFACHUNG + MITTEL-ANKER (Operator 2026-06-11): NUR die zwei ENDEN sind OBJEKTIVE Aussagen (>1 P/m² = „objektiv voll/rot"; ~leer = „objektiv frei"); DAZWISCHEN sagt das System nichts Objektives — die Mitte ist die ENTSCHEIDUNGS-ZONE, wo die Comfort-LINIE des Users entscheidet. → man muss NICHT für jede Mittelstufe eine Dichte-Wahrheit festnageln; zwei Enden fix + lineare Interpolation, niemand erhebt objektiven Anspruch auf die Mitte (nimmt genau die Last weg, mit der der Operator rang). FEINHEIT (Relativität nicht versehentlich reinholen): das MESH färbt die Mitte trotzdem ABSOLUT (gegebene Dichte → IMMER dieselbe Farbe, Interpolation fix); „Comfort-Situation" = persönliche Schwelle entscheidet, NICHT subjektive Farbe. Operator-Bild (exakt): die Comfort-Linie ist eine FESTE Marke auf einer FESTEN Skala; was wandert, ist nur WO die Tatsachen relativ zur Linie liegen = Thermostat (Zahlen am Rad fix, Raumtemperatur variiert). — SICHTWEITEN-ANKER für die PATH-Skala (Operator-Idee): die lineare Dichte über einen MENSCHLICHEN Maßstab verankern statt abstrakte P/m² (= relationale Kalibrierung ann_124): Sichtweite ~150–200 m (geländeabhängig: offener Weg ~200, Wald weniger) = die natürliche Einheit für „allein vs Gesellschaft". LEITER: nächste Person >1 Sichtweite weg (>200 m) = praktisch allein = Stufe 1 · ~1 Sichtweite Abstand = „ich habe Gesellschaft" · enger → höher bis Schulter-an-Schulter oben. Claude-Schärfung: Sichtweite markiert eher die Grenze ALLEIN↔GESELLSCHAFT (~Stufe 1,5–2,5) als die satte Mitte; echte Mitte (Stufe 3) dichter (~alle 50–80 m einer) — die EXAKTE Zahl am echten Netz feinjustieren, die Sichtweite gibt den Ankerpunkt. Gilt für PATH (linear); REST (Fläche) behält den P/m²-Anker. — ABSTANDS-LEITER PATH (Operator 2026-06-11, Erst-Kalibrierung, beantwortet die „wie von 3 auf 5 ohne Stufe 6?"-Frage SELBST): Stufe 3 = 250 m Abstand · Stufe 4 = 150 m · Stufe 5 = 50 m · Stufe 6 = 1 P/m² (Decke). Stufe 0,5–3 = einfach (leer-ish). → die Mittel-/Oberstufen sind ABSOLUT über den Abstand verankert, existieren AUS SICH HERAUS, brauchen Stufe 6 NICHT erreicht (Stufe 6 bleibt an ruhigem Tag leer) — nichts wird „hochgerechnet", keine Relativität. SLIDER: Default mittig (Stufe 3 = 250 m); „unsichtbarer Anschlag nach oben / nicht mehr Unruhe als eingestellt" = die Comfort-Schwelle selbst (der Slider IST die Bremse); LIVE-MARKE „so voll ist es gerade" am Slider (aktuelle Auslastungs-Stufe neben der Einstellung — zeigt Realität gegen die eigene Linie). Zahlen = feinjustierbar am echten Netz. — STATUS: reine Formulierung (kein Bau). Bezug: ann_123 (Perzentil für die MESSUNG verworfen, Telco-absolut bestätigt), ann_124 (Thermostat; „relational" = Präferenz), ann_127 (Path/Rest = die zwei Einheiten), ann_126 (Ignore-Episode = gleiche stateful-Idee).',
    related_panel: 'P01',
    date: '2026-06-11',
  },

  // ── PATH vs REST — Zwei-Kernel-Architektur (Radien zerschneiden das Mesh) (2026-06-11) ──

  {
    id: 'ann_127',
    category: 'invariant',
    label: 'PATH vs REST — Zwei-Kernel-Architektur (Radien zerschneiden das Mesh; Rest vor Path)',
    content: 'FUNDAMENTALE ARCHITEKTUR-NOTIZ (Operator 2026-06-11, von Claude gespiegelt) — die Last zerfällt in ZWEI UNABHÄNGIGE WELTEN, geometrisch getrennt durch RADIEN um die POIs, die das MESH ZERSCHNEIDEN: INNERHALB eines POI-Radius = RAST-ZONE (REST) · AUSSERHALB = GEHEN (PATH). „Hier wird gerastet, ausserhalb der Radien wird gegangen." Was INNERHALB der Radien passiert, ist der PATH-Welt EGAL. → das macht den ann_122-Split STAU(auf Strecken)/RAST(um POIs) KONKRET: der Radius IST das Werkzeug, das die Mesh-Segmente in Rest-Zonen vs Path-Strecken partitioniert; die Last wird entsprechend gespalten (Rest-Last je POI/Zone, Path-Last je Strecke). — ZWEI KERNEL: BCK/BAK-PATH (Stau auf den Geh-Strecken) + BCK/BAK-REST (Rast um die POIs). JEDER hat einen EIGENEN COMFORT (Slider). ⚠ EXISTIERT IN DER UI BEREITS: der ComfortSliders trägt schon zwei Achsen — WEG/movement (= PATH) + RAST/stay (= REST). Die UI-Primitive ist da; NEU sind nur die GETRENNTEN LAST-MODELLE + Schwellen dahinter. — ZWEI MINI-MODALE (vergleichbar, je eines): PATH-Modal + REST-Modal. ⚠ KLARSTELLUNG (Operator): die GANZE bisherige POI-Entscheidungs-Diskussion (director-controls, auslassen/ersetzen, ann_126) IST die REST-Seite („über das haben wir die ganze Zeit gesprochen"). PATH = Strecken-/Kreuzungspunkt-Entscheidungen = das post-MVP-„C" (eigene Strecken-Wahl). — PRÄZEDENZ (das Neue): REST GEWINNT IMMER VOR PATH, weil über REST die ZIELE (POIs) formuliert werden. Das System klärt ZUERST Rest (welche POIs), DANN Path (Route dazwischen) — ZIELE VOR ROUTE. Logisch zwingend: erst die Destinationen, dann der Weg. — REORGANISATION der bestehenden Kaskade durch diese Linse (heute sind beide VERMISCHT, konzeptuell zu trennen): Stufe-1 Auto-Ausweichroute (solveRouteAvoiding, belebte Strecken meiden) = PATH (autonom) · Stufe-2/3 POI auslassen/ersetzen = REST (interaktiv). — UNABHÄNGIG + ORDENTLICH ZUSAMMENLAUFEN (Operator-Schluss): „wahrscheinlich zwei völlig getrennte, unabhängige Berechnungsmodelle, die in der UI GEORDNET zusammenlaufen, nicht um den User KONKURRIEREN müssen." → richtige Architektur: zwei Kernel (separation of concerns) + EINE geordnete UI, die sie SEQUENZIERT (Rest-Entscheidungen zuerst, dann Path). Die Gefahr (zwei Prompts konkurrieren um den User) ist durch die PRÄZEDENZ+Sequenz gebannt. — KONFLIKTPOTENZIAL (Operator benannt): Rest+Path unabhängig → der User müsste ggf. BEIDE justieren, um in Ruhe gelassen zu werden. ABER: Path ist system-seitig AUTONOMER. — MVP-VEREINFACHUNG (Schlüssel): im MVP kann man GAR KEINE Strecken wählen → PATH ist VOLL AUTONOM → nur REST ist interaktiv → effektiv NUR EIN Mini-Modal (Rest). Path-Interaktion (zweites Modal) = post-MVP. — „SCHWIERIG ODER DOCH NICHT?" (Operator-Frage zu: bei POI-Justierung muss das System auch Paths bewerten) → NICHT so schwierig, WEIL die Präzedenz es ordnet: man bewertet Path NICHT WÄHREND der POI-Justierung. Sondern: User justiert POIs (Rest) → System re-solvt die Route automatisch (macht es schon: solveRoute/solveRouteAvoiding) → ERST DANACH flaggt der Path-Comfort, falls die resultierende Route Strecken zu voll macht (= Path-Mini-Modal IM ANSCHLUSS). Die Sequenz löst die scheinbare Schwierigkeit auf — kein Wettstreit, ein Downstream-Schritt. — PRÄZISIERUNG #2 (Operator 2026-06-11): (1) RADIUS = LAST-ABHÄNGIG + GEKAPPT: der Rast-Radius wächst mit der Rest-Last des POI (dynamisch/elegant), ABER hartes MAXIMUM, damit das System nicht durchdreht — ~20 m ist schon SEHR groß (Obergrenze in der Gegend). Sinnvoll auch ein MINIMUM (jedes POI hat immer einen kleinen Rast-Schatten, auch leer). (2) HIERARCHIE = PATH GATET FEASIBILITY (löst „wozu die Rest-Last eines POI, das die Path-Last gar nicht zugänglich macht?"): EINE REGEL — „ein POI entscheidet NICHT selbst, ob es gewählt werden kann; zunächst entscheidet die WEGLAST (Path), ob es überhaupt WÄHLBAR ist." Das ist KEINE Umkehr der Rest-vor-Path-Präzedenz, sondern eine ANDERE EBENE: PATH = CONSTRAINT/ERREICHBARKEIT (Gatekeeper: ist der POI über die Strecken comfort-konform ERREICHBAR? — oben in der Feasibility-Hierarchie = BCK-Path) · REST = INTENT/WAHL (innerhalb der erreichbaren Menge resolved der User seine Ziel-Entscheidungen zuerst — oben in der Interaktions-Präzedenz). MERKSATZ: PATH FILTERT DAS MENÜ, REST WÄHLT DARAUS. → löst das „Konfliktpotenzial" der Vormessage AUF: sie streiten nie um dasselbe — Path begrenzt den Möglichkeitsraum, Rest wählt darin. Ein POI hinter zu voller Strecke ist NICHT im Menü; seine Rest-Last wird erst relevant, wenn es den Path-Gate passiert (= die gefühlte Systemhärtung). (3) ZWEI ANTI-FLACKER-SCHICHTEN (Operator-„Denkfehler" war nur deren Vermischung, kein Fehler): (a) HYSTERESE/Halbstufe = SENSING-Ebene (Last kreuzt Comfort-Schwelle ohne zu flattern, 0,5-Band, ann_123) · (b) COMMITMENT-STICKINESS = ENTSCHEIDUNGS-Ebene (hat der User gewählt, stellt das System die Wahl NICHT in Frage, wenn die alte Last wieder sinkt = ann_126 „nach Commit raus, auch wenn Last sich normalisiert"). Beide längst im Modell, getrennte Ebenen; das Comfort-2/Last-3-Beispiel ist exakt die Stickiness. — STATUS: reine Formulierung (kein Bau); gehört zur shell-kit-Wurzel-Logik vor dem Umbau. Bezug: ann_122 (Stau/Rast = diese Path/Rest-Spaltung), ann_126 (Redesign = die REST-Seite), ann_123/124 (Comfort/Thermostat, gilt je Kernel getrennt).',
    related_panel: 'P06',
    date: '2026-06-11',
  },

  // ── REDESIGN — On-Map-POI-Sprache + Director-Controls (Manege-Ablösung) (2026-06-11) ──

  {
    id: 'ann_126',
    category: 'next_intent',
    label: 'REDESIGN — On-Map-POI-Sprache + Footer Director-Controls (Manege/Modals ablösen)',
    content: 'GROSSER UX-REDESIGN (Operator 2026-06-11, zu präzisieren+abzuarbeiten, NICHT überstürzen) — die KARTE-VERDECKENDEN Modals (BAK-Stufe 1/2/3) + die MANEGE (Karte-in-Karte) werden ABGELÖST. Grund: sie verdecken die Karte und blockieren den IMMER justierbaren Comfort-Slider; die Manege ist „zu viel". Die LOGIK (dompteurPicks, breachingLegs, dimmed, legColor, Severity) BLEIBT — es ist ein PRÄSENTATIONS-Umbau. — (1) ON-MAP-POI-SPRACHE (Gestalt = Größe+Deckkraft+Hektik, statt Modal-Farben): jedes POI hat eine Gestalt. Unattraktiv ≠ uninteressant — die UMSTÄNDE (zu belebt/eng) machen es unattraktiv; „eng" = alles wird KLEIN. Skala: DEGRADIERT (außerhalb Comfort/unattraktiv) = Basis ×1.0, 90 % Deckkraft (heute mehr → ändern); IM-COMFORT = ×1.2, opak; ROUTE-POI = ×1.6 (ggü. degradiert); BELEBTES Route-POI = HEKTISCH (je voller, desto hektischer — last-eskaliert, die gebaute Nervös-Animation); ABGEWÄHLT → degradiert 90 % + klein + raus aus Route. — (2) AUSWAHL NUR IN DER MAP (Tap = an/abwählen; ist schon so). Keine Karte darf die Map verdecken AUSSER der POI-INFO-Card (long-tap-Detail / S4-Ankunft = die eine erlaubte Overlay-Karte). Bei Problemen: Karten-AUTOFOCUS auf die betroffenen POIs (statt Manege-Overlay) + Platz für Info/Gesamtabwahl. — (3) ENTSCHEIDEN = FOOTER-CONTROLS (volle Breite, „Director-Controls" — der User ist der Director): Routen-Entscheidungen = director-controls; die UHR = Ergebnis, sitzt RECHTS IN den Controls; der PLAY-Button visuell RECHTS AUSSEN. Ein DROP-UP klappt nach oben auf (wächst höher, wenn mehrere POIs betroffen). Zwei MODI: AUTO (System entscheidet still was rausfällt, nichts hektisch, Map sagt nur „POI xy umgangen") · MANUELL (betroffene POIs werden hektisch; Info je POI mit 3 Zuständen: IGNORIEREN · RAUSNEHMEN (Dauer mögl. gleich) · ERSETZEN (Dauer nachrangig)). — (4) PREVIEW + COMMIT: die Controls legen nur die ABSICHT fest, die Karte zeigt die PREVIEW LIVE; erst der PLAY-TAB schließt das Drop-up + COMMITTET; Play BLINKT solange eine Entscheidung offen ist; nach Commit ist das POI raus, AUCH wenn die Last sich wieder normalisiert (sonst zu kompliziert). Bevorzugt: Preview INTEGRIERT (ohne eigenen Preview-Button) — „schöner". — (5) ERSETZEN-LAYOUT im Drop-up (nutzt die gebaute Vorschlags-Kaskade): LINKS das hektische Alt-POI (Tap = DELETE, durchgestrichen), RECHTS davon 3 ruhige Alternativen nebeneinander (einzeln/keiner wählbar, wechseln+nennen %/±Dauer; Map ändert sich live), noch weiter rechts AUTO-COMFORT. Unter allen POIs immer ±Zeit, Gesamtzeit live. 4 POIs nebeneinander LINKS DER UHR (geht sich im Footer aus). Titel statt alt/neu → besser INTAKE/IN vs OUTTAKE/OUT, Tabler-Icon-Style (Icon für off + Icon für intake). — OFFEN/FUZZY: Preview-Integration vs -Button (Tendenz integriert); 4-POI-Footer-Layout auf Mobile = eng (Layout-Herausforderung); genaue Drop-up-Gestalt UI/UX; Interaktions-Details. — WARUM MANEGE GEBAUT + jetzt verworfen: war die kollektive Überlast-Entscheidung laut früherer Spec; jetzt erkannt: Map-Overlay = zu viel. „Manege" als Begriff evtl. neu verorten. — TESTS NÖTIG: Routenbildung/Alternativ-Vorschlag/Auswahl könnten unausgereift/verwirrend sein → echte Tests. Wege müssen EINFACH + KLAR veränderbar sein. — — PRÄZISIERUNG #2 (2026-06-11): (a) ERSETZEN IST REVERSIBEL: Tap aufs hektische Alt-POI toggelt hektisch↔degradiert (beliebig oft). Erst die ABWAHL (Delete) des hektischen erzeugt die 3 INTAKE-Vorschläge; solange NICHT abgewählt, steht an Stelle der 3 Kandidaten nur das Label „Alternativ POIs" (Platzhalter). Intake-Wahl ebenfalls reversibel. (b) Preview INTEGRIERT (kein eigener Preview-Button) — bestätigt. (c) Im expandierten Footer hat TEXT keinen Platz mehr (2 Textzeilen entfallen); UHR auf ×0.8. (d) FOOTER-POIs öffnen per LONG-TAP dasselbe Info-Modal wie Map-POIs. (e) FOOTER-RASTER = 7 Spalten: 4 POI + Uhr (=2 Spalten) + Play (=1) — geht sich genau aus. (f) Play liegt AUSSERHALB → Uhr darf visuell eigenständig in ihrer Box sein. (g) Die linke 4-POI-Box EXPANDIERT NACH OBEN, je weiterem hektischen POI (eine Zeile je hektischem POI). (h) ZWEI INTERAKTIONS-EBENEN + SYNC (Kern!): Director-Controls = PREVIEW (Absicht, committet bei Play); ABER Eingriff IN DIE KARTE = REALE Routen-Änderung (keine Preview) → die Director-Controls müssen sich der neuen Lage ANPASSEN (re-derive). — ZUSTANDSKERN (Claude-Vorschlag, gehört in die shell-kit-WURZEL, NICHT CSS): (1) POI-ZUSTAND (rein abgeleitet aus onRoute? + Comfort/Last + busy?, eine Kernel-Funktion poiVisualState): degradiert (außerhalb Comfort, nicht Route) ×1.0/90% · imComfort ×1.2/opak · onRoute ×1.6/opak · onRouteBusy ×1.6/HEKTISCH(last-eskaliert). (2) ROUTEN-ZUSTAND (die Maschine): committedRoute (real) ⊕ intent (offene Footer-Entscheidungen) → previewRoute (=committed+intent, auf der Map gezeigt). Map-Tap = SOFORTIGE Real-Edit von committedRoute (+ intent re-based); Footer baut intent; Play COMMITTET intent→committed (Play blinkt solange intent offen; nach Commit POI raus, auch wenn Last sich normalisiert). (3) REPLACE-FLOW je hektischem POI: kept↔deleted (Tap, reversibel); bei deleted → 3 Intake-Kandidaten (dompteurPicks) erscheinen, Tap=intake (reversibel), sonst Platzhalter „Alternativ POIs". → Runtime/CSS RENDERT nur diese Zustände (Größe/Deckkraft/Hektik) + verdrahtet Gesten; die LOGIK lebt im KERNEL (shell-kit). — REIHENFOLGE (Operator): erst die GANZE neue Logik PRÄZISE durchdacht+formuliert, DANN Umbauplan; der Umbau wirkt an der shell-kit-WURZEL (kein CSS-Hokuspokus) = die eigentliche Herausforderung. — PRÄZISIERUNG #3 (2026-06-11): ALTERNATIV-POIs = WASSERZEICHEN, NICHT TEXT: die 3 Intake-Slots zeigen die Kandidaten von Anfang an als blasse Geister/Wasserzeichen (inaktiv); erst das LÖSCHEN des hektischen POIs AKTIVIERT sie (voll, selektierbar). Vorteil: zeigt vorab was bereitsteht, lehrt die Regel „erst weg, dann wählen", erklärt warum so viele POIs. (Option: dieselben Geister auch auf der Karte an ihren echten Orten andeuten.) — ROUTENMASCHINE — ZUSTÄNDE+ÜBERGÄNGE (präzisiert): ZWEI ZUSTÄNDE: LIVE = committedRoute sichtbar, KEIN Rahmen (bei Problem: Drop-up zeigt es, Play blinkt) · PREVIEW = Footer-Absicht gebaut → Karte zeigt previewRoute + RAHMEN („Vorschau") + Play blinkt. ÜBERGÄNGE: in PREVIEW Play-Tap → intent→committed, Rahmen weg, NEU ERKENNEN → LIVE; in PREVIEW/LIVE Map-Eingriff → committedRoute ändert SOFORT (real) → JEDE offene Absicht ANNULLIERT, Rahmen weg, NEU ERKENNEN → LIVE. EINE REGEL (löst „verschwinden vs verändern"): Map-Eingriff annulliert + leitet Director-Controls aus der NEUEN Lage NEU AB (Problem weg → Drop-up schließt; neues/anderes Problem → Drop-up zeigt das). RAHMEN = präziser Indikator: an=PREVIEW (Absicht offen), aus=LIVE. BEGRÜNDUNG (Operator): die Absicht ist RELATIV zur Problem-Lage; Map-Eingriff ändert die Lage → Basis weg → Absicht verfällt, anders ergäbe es keinen Sinn. — ENTSCHIEDEN (Operator 2026-06-11): die EINFACHE Regel — ein USER-EINGRIFF IN DIE MAP ANNULLIERT JEDE User-Absicht der director-controls (nicht nur die betroffene). „Map = ich übernehme direkt." (Die granulare „nur betroffene"-Variante ist vom Tisch, höchstens spätere Verfeinerung.) → ROUTENMASCHINE damit VOLLSTÄNDIG spezifiziert. — PRÄZISIERUNG #4 (2026-06-11): (1) AUTO-MODUS GESTRICHEN: kein wählbares AUTO (zu bevormundend, macht das System verantwortungsträger; bei klarer UX keine Vereinfachung sondern Doppel-Unsicherheit). Die EINZIGE Automatik = Stufe-1-Deeskalation (stille Ausweichroute, läuft ohnehin). → AUTO-COMFORT-Button im Replace-Layout entfällt wohl auch (User wählt; Operator bestätigt noch). (2) EINTRITT = MINI-MODAL statt auto-poppender Controls: bei Problem + Play-blinkt erscheint klein „Besucherdruck: Deine Route ≠ deinem Comfort" + [CHANGE] [IGNORE]. CHANGE → Modal weg, Map-AUTOFOCUS auf betroffene POIs, Preview-Rahmen, director-controls poppen auf. IGNORE → Modal weg, POI(s) flippen in der Map weiter, Play blinkt NICHT mehr (busy akzeptiert). (3) POI-INFO-MODAL (long-tap/Ankunft): Icon + Titel + Kurzbeschreibung (jetzt; später Langbeschr./Foto/Link); Tap schließt. ⚠ DATEN-DEP: Bundle-POI trägt heute nur text(Name)+subcategory → Kurzbeschr./Foto/Link müssen ins Origin-POI-Set. (4) POI-GESTALT VOLLSTÄNDIG (Grundsatz: ALLE out-of-comfort POIs FLIPPEN/zappeln): out-of-comfort+off-route = ×1.0 + leicht gedimmt + zappelt(klein); im-comfort+off-route(„normal") = ×1.2 ungedimmt; im-comfort+on-route = ×1.6 ungedimmt; out-of-comfort+on-route(HEKTISCH) = ×1.6 ungedimmt + zappelt(groß); ALTERNATIV-Kandidat VOR Aktivierung = ×1.2 als 50%-WASSERZEICHEN; NACH Delete des Zappel-POI aktiviert = ×1.2 normal; SELEKTIERT/intaken = ×1.6. Tap aufs hektische On-Route-POI = Delete → wird zum DEGRADIERTEN (off-route, natürlicher Zustand klein+gedimmt+zappelt falls weiter busy), NICHT Wasserzeichen (Wasserzeichen NUR für Alternativ-Kandidaten). Tap ändert visuell: linkes POI + Preview-Route + betroffenes Map-POI. (5) OFFEN — BEGEHUNGS-WECHSELWIRKUNG: heute friert die Kaskade während Play/Begehung ein (Route nicht unter den Füßen ändern); offene Frage fürs neue Modell: Live-Re-Eval WÄHREND des Gehens ja/nein (mächtig, kollidiert mit „nicht unter Füßen", hängt an S5/echtem GPS = post-MVP). Fürs MVP: eingefroren während Play. (6) FLACKER-HYSTERESE (Deadband/Thermostat) = BCK-Kernel-Gap, siehe ann_123 (heute nur eine Schwelle). — PRÄZISIERUNG #5 (2026-06-11, Operator-Bestätigungen): AUTO-COMFORT-Button GESTRICHEN (bestätigt) → Replace-Row = hektisches POI + 3 Alternativen = genau die 4 POIs. BEGEHUNGS-WECHSELWIRKUNG: das VOLLE Modell IST live-im-Gehen (Mini-Modal + director-controls erledigen Probleme rasch+übersichtlich WÄHREND der Begehung — so war es immer gedacht), aber DEFERRED auf S5; MVP bleibt eingefroren während Play. Anti-Schrei-Schutz = die 0,5-Hysterese. IGNORE = Meldung verschwindet + kommt erst wieder, wenn die Last ERNEUT STEIGT (über das ignorierte Niveau) → System muss das ignorierte Last-Niveau je Problem MERKEN (Re-Alert nur bei Überschreiten). MINI-MODAL zeigt das betroffene POI (man sieht es flippen + Eindruck des Füllstands), NICHT interaktiv (Interaktion = director-controls). Wasserzeichen NUR für Alternativ-Kandidaten (bestätigt). — IGNORE-LEBENSDAUER (Operator 2026-06-11, ERSETZT das komplexe Last-Niveau-Gedächtnis aus #5): das „merke das ignorierte Last-Niveau, re-alert bei Überschreiten" wird VERWORFEN (zu komplex zu bauen + Gültigkeitsdauer unklar — wie der POI-Radius ein Fass). PRAKTISCHE REGELN: Ignore löste KEINE Preview aus; ein nachfolgendes PREVIEW überschreibt jeden Ignore-state; eine MANUELLE POI-ABWAHL in der Map überschreibt den Ignore-state (deckt sich mit „Map-Eingriff annulliert alles"). OFFENE Operator-Frage „was, wenn KEINE Preview/kein Map-Eingriff stattfindet?" → ANTWORT (Claude, memory-frei, analog zur Hysterese-Episode): Ignore an die laufende BREACH-EPISODE binden — unterdrückt GENAU diesen Engpass, solange er anhält; sobald die Last wieder IN den Comfort zurückfällt (Episode endet), STIRBT die Unterdrückung mit ihr; ein ERNEUTER Breach = neue Episode = darf wieder melden. → braucht WEDER Last-Niveau-Gedächtnis NOCH TTL: Ignore-Lebensdauer = Lebensdauer des aktuellen Breachs (gleiche Thermostat-Episoden-Logik wie das 0,5-Deadband ann_123). Breach bleibt + User tut nichts → bleibt still (gewollt). Bezug: ersetzt die Präsentation von B2/B3 (ann_119/guidance_play_bauplan.md); Logik bleibt.',
    related_panel: 'R07',
    date: '2026-06-11',
  },

  // ── Comfort-Adjust als Stau-/Fluss-Sensor (Telco-unabhängig) (2026-06-10) ──

  {
    id: 'ann_125',
    category: 'adr',
    label: 'KERN-INSIGHT — Comfort-Adjust als Stau-/Fluss-Sensor (ohne Telco/GPS)',
    content: 'KERN-INSIGHT (Operator 2026-06-10, von Claude geschärft) — DER COMFORT-ADJUST IST EIN SENSOR; löst die ann_122-Lücke „gerichtete Ströme" OHNE Anthem-Schema, OHNE Telco, OHNE GPS. — KERN: senkt ein User unterwegs den Regler, meldet er ERLEBTEN Andrang („hier ist es mir zu viel") = BODENWAHRHEIT, nicht eine gemessene Zahl, sondern die Sache selbst. — DIRECTIONAL vs BIDIREKTIONAL OHNE TELCO/STANDORT: das Gerät kennt edge-lokal die ROUTE → die RICHTUNG des Users. Aggregiert man pro Segment (ANONYM) die Unbehagens-Signale + die Richtungen der Betroffenen: mehrheitlich EINE Richtung → directional (Einbahn-Surge); BEIDE Richtungen → bidirektionale Verdichtung. Braucht NUR Comfort-Daten + geräte-lokale Richtung, KEINE GPS-Standortdaten (Lokalisierung aus der ohnehin bekannten Route, anonym je Segment → Black-Box/Datensparsamkeit-konform). — EXTREMSITUATIONS-THESE (das Entscheidende): DICHTE ≠ STAU. Hohe Dichte kann FLIESSEN (harmlos) oder STOCKEN (Stau/Leid). Telco-Dichte gibt eine Zahl, unterscheidet das NICHT → verwechselt blind Dichte mit Stau. Der Comfort-Adjust verwechselt es NIE: misst direkt das Leid + (über die Richtung) das Fluss-Muster. „Ein System entscheidet sich im Extrem" — im Extrem senken die Leute MASSENHAFT den Regler + das Richtungs-Signal leuchtet auf = der Moment, für den das Design gebaut ist; die naive Dichte verliert genau da. — EHRLICHE GRENZE: ABDECKUNG — der Sensor sieht nur, wo App-User SIND; bei dünner Adoption spärlich → Telco bleibt als BREITEN-/Coverage-Schicht nützlich. Aber die ENTSCHEIDUNGS-QUALITÄT (gerade im Extrem) kommt vom Comfort-Adjust. — ROLLEN: Comfort-Adjust = WAHRHEITS-/KALIBRIER-Schicht (Ground Truth = erlebtes Leid, NOCH wahrer als Dichte; Wahrheitskreislauf/TL in Reinform; passt zu ann_124 „Mensch kalibriert, nicht die Abstraktion"); Telco (optional) = Breiten-Schicht. — AUFLÖSUNG der gefühlten „Kollision": der Konflikt war gemessene ABSTRAKTION vs erlebte WAHRHEIT — gelöst, indem die Wahrheit zur QUELLE wird statt aus Dichte aufs Leid zu raten. — DATEN: braucht NUTZER-COMFORT-DATEN (Adjusts + geräte-lokale Richtung, anonym aggregiert), KEINE Standortdaten. — ⚠ STATUS = FÄHIGKEIT, NICHT REALITÄT (Operator-Korrektur): das System GENERIERT diese Signale HEUTE NICHT, es KÖNNTE sie nur generieren. Fehlt: (1) AUFWÄRTS-KANAL — Comfort-Änderungen bleiben lokal am Gerät, nichts wird (anonym je Segment) eingesammelt; das Anthem fließt nur RUNTER (Last→Gerät), nicht RAUF. (2) Kritische MASSE an Usern. (3) Signal feuert nur bei AKTIVER Justierung (passive User = stumm). — FOLGE = REIFEBOGEN (statt „Telco optional"): BOOTSTRAP (Tag 1, null User) → nur TELCO sieht etwas (ohne sie blind); WACHSTUM → Comfort-Signal sammelt sich an; REIFE → Comfort-Sensor übernimmt als Wahrheits-Schicht, Telco bleibt Breiten-Stütze. Telco ist also der ANSCHUB, bis der Sensor geboren ist — die zwei sind AUFEINANDERFOLGEND, nicht Alternativen. — MEHR SIGNAL-QUELLEN als nur der Slider: jedes „POI auslassen" (Stufe 2), jede Manege-Entscheidung „belebte Ziele weglassen", jeder Route-Edit IST ein latentes Comfort-Signal — die gebaute UX PRODUZIERT sie schon; es fehlt nur der Aufwärts-Kanal + das anonyme Aggregat, NICHT das Verhalten. — HYBRID (Operator-Formel): GROBSORTIERUNG = Telco · FEINJUSTIERUNG = Anonym-User-Adjust → gemeinsam die relative User-Verteilung. — MVP-LICHTENBERG: NICHT richtungsentscheidend (MVP läuft auf Telco/Sim; Comfort-Sensor = post-MVP, braucht Aufwärts-Kanal + User-Masse). Einzige MVP-Relevanz: die TÜR NICHT ZUMAUERN. — VORZUBEREITEN (billig, KEIN Bau): Comfort + Kaskaden-Entscheidungen (Auslassen/Manege/Route-Edit) als diskrete Events mit SEGMENT+RICHTUNG-Kontext halten (Gerät kennt beides) + anonymisierbar; einen Aufwärts-Kanal im Anthem-Architektur-Bild RESERVIEREN (heute down-only), nicht bauen. — ARCHITEKTUR-AUSSAGE (zu dokumentieren): (a) Reifebogen Telco-grob→Comfort-fein als expliziter Pfad; (b) Aufwärts-Kanal = ANTIZIPIERTE Zukunfts-Komponente (Anthem heute nur abwärts); (c) Privacy: anonyme per-Segment-Aggregate, KEINE Standortdaten (Black-Box-konform); (d) MVP nutzt Telco/Sim, Comfort-Sensor post-MVP. STATUS: noch genauer zu betrachten (Operator, später). Bezug: ann_122 (Lastmodell/Ströme), ann_123 (Comfort), ann_124 (relationale Kalibrierung).',
    related_panel: 'P06',
    date: '2026-06-10',
  },
...

  {
    id: 'ann_122',
    category: 'invariant',
    label: 'FUNDAMENTAL — Lastmodell: Spinnennetz · gerichtete Ströme · Stau/Rast',
    content: 'FUNDAMENTALE DESIGN-NOTIZ — LASTMODELL (2026-06-10, Operator). (1) REVIER: Wir arbeiten am STRUKTURELLEN SPINNENNETZ (Wegnetz). Routen OHNE Netz — wie ein einzelner Pfad auf einen Berggipfel — sind ohnehin NICHT unser Revier (kein Ausweichen möglich = nichts zu optimieren). (2) BEWEGUNGSSTRÖME: nicht nur BIDIREKTIONAL (Hin/Rück), sondern auch ONE-DIRECTION-LASTSTRÖME (z. B. Events, Einbahn-Andrang am Berg) — diese GEHÖREN UNBEDINGT berücksichtigt. ⚠ HEUTE trägt das Anthem nur SKALARE Segment-Last (richtungslos, loads[] je Segment-Index) → für gerichtete Ströme braucht es eine künftige ANTHEM-SCHEMA-Erweiterung (gerichtete Last je Segment). (3) ZWEI LAST-ARTEN, die das System erkennen MUSS: STAU (auf Strecken/Segmenten) und RAST (Verweilen um POIs). Heute mischt die Segment-Last beides nicht auf — künftig trennen. (4) LAST-BEZUG: pro m² maximal ~1 Person (Sättigung). OFFENE UX-FRAGE: wie kommuniziert man „Last" über hunderte m Wanderweg verständlich/intuitiv? — Bezug: Comfort-Individualisierung ann_123.',
    related_panel: 'P06',
    date: '2026-06-10',
  },

  // ── Comfort-Individualisierung + Anthem-Daten (Konzept + offene Frage) (2026-06-10) ──

  {
    id: 'ann_123',
    category: 'next_intent',
    label: 'Comfort-Individualisierung — Perzentil der Live-Last-Verteilung (Konzept)',
    content: 'COMFORT-INDIVIDUALISIERUNG (Konzept + Claude-Vorschlag, 2026-06-10). — ZIEL: jeder User erfährt den Comfort, den er GLAUBT eingestellt zu haben; niemand wird je enttäuscht, weil insgesamt jeder in einer ihm passenden SPANNE wandert. Das System soll lernen, mit Comforteinstellungen so umzugehen, dass das gelingt. — UI: 6 SICHTBARE Stufen (intern 12), ABER das System soll JEDEM eine wirklich INDIVIDUELLE (faktisch kontinuierliche) Einstellung gewähren können. — MECHANIK „IN DIE WÜSTE": der User mit der höchsten Empfindlichkeit (niedrigste Last-Toleranz) ist der ERSTE, dem die Last zu viel wird → er wird in den unbelebtesten Winkel umgeleitet, den das System kennt. Wer unterwegs überfordert ist + seine Comforteinstellung senkt, wird so einfach in ein ruhigeres Gebiet „umgeleitet". — CLAUDE-VORSCHLAG (analog zum Animations-Beispiel „Range → Charakterisierung → innerhalb"): Comfort NICHT als ABSOLUTER Last-Schwellwert führen, sondern als PERZENTIL der LIVE-LAST-VERTEILUNG der Rep. Mechanik: SCIM3 holt aus dem Anthem die per-Segment-Lasten (HAT es schon, loads[]) → rechnet daraus die aktuelle Last-VERTEILUNG (min/max/Perzentile bzw. kleines Histogramm) → die Comfort-Stufe/der individuelle Wert mappt auf ein PERZENTIL → ein DYNAMISCHER, RELATIVER Schwellwert „so viel Andrang toleriere ich, gemessen an dem, was gerade verfügbar ist". VORTEIL: der Ruhe-Suchende bekommt IMMER das Ruhigste-Verfügbare, egal wie sich die absoluten Lasten verschieben; das „in die Wüste" fällt natürlich heraus (niedrigstes Perzentil = stillster Winkel); jeder wandert in seiner Spanne → keine Enttäuschung. — WELCHE ANTHEM-DATEN BRAUCHT SCIM3? BASIS: KEINE NEUEN — nur die VERTEILUNG aus den vorhandenen loads[] (Black-Box-/Edge-konform: am Gerät gerechnet). RICHERE Stufe (gerichtete Lastströme, Stau-vs-Rast-Trennung, siehe ann_122) → künftige Anthem-Schema-Erweiterung. — OFFEN: Lernkomponente präzisieren; Last-Kommunikation über hunderte m (ann_122 (4)); ob das alte „Empfindlichkeitszahl/Stufe"-Konzept (Operator nicht mehr geläufig) hier andockt. — OPERATOR-VERFEINERUNG (2026-06-10): UNTERSCHEIDE (a) den EINGESTELLTEN Comfort-GRUNDWERT (nur 1–6) von (b) dem LIVE-AKT „ich will es JETZT ruhiger" (User senkt unterwegs). Bei (b) greift das System und schickt ihn in die NÄCHST-unbelebtere Zone von 6 — intern 12 Stufen (Zwischenstufen gegen Flackern / zur Interpolation) —, AUSSER er verlangt (unbewusst; die UX ist eine Fein-Einstellung) 2 Zonenschritte. KALIBRIER-IDEE: jeder hat eine Grundeinstellung 1–6; sobald der ERSTE mit Stufe 1 es NOCH ruhiger will, ist die System-Untergrenze erreicht = das System ist kalibriert. Deckt sich mit dem Perzentil-Modell: Stufe 1 = niedrigstes Perzentil = stillster verfügbarer Winkel; sein Mehr-Wunsch markiert den BODEN der Verteilung. So ist „Grundwert" = Position in der Verteilung, „Live-Senken" = ein Perzentil-Schritt nach unten. — ANTHEM-DATEN + FLACKER-SCHWELLE (geklärt 2026-06-11, CODE-VERIFIZIERT): ANTHEM liefert je Segment EINEN KONTINUIERLICHEN Last-Float (0..1; simSegmentLoads→number[], stretchAverages→average 0..1), NICHT 6 oder 12 diskrete States. Die 6 (sichtbar) / 12 (intern) sind eine COMFORT-/UI-QUANTISIERUNG OBEN DRAUF (Colour-Mesh-Felder, Comfort als Position), NICHT etwas das Anthem ausliefert. ANTI-FLACKER = HYSTERESE/DEADBAND (= THERMOSTAT, ann_124): Comfort 3/6 → Segment „breacht" erst ÜBER 3,5 (oberer Halbstep = das 12.-Level) und hört erst UNTER 3,0 wieder auf (Eintritts- ≠ Austritts-Schwelle, 0,5-Band) → kein Flattern am Rand. ⚠ CODE-GAP (Operator-Sorge bestätigt): heute `dimmed = average > comfort` = EINE Schwelle, KEINE Hysterese, KEINE 6/12 → genau der ON/OFF-SWITCHER, den wir NICHT wollen. Die Hysterese (macht Sensus Core erst zum Thermostat) ist NOCH NICHT gebaut = echte BCK-Kernel-Arbeit. — KLÄRUNG TELCO vs PRÄFERENZ (2026-06-10, NICHT fusionieren!): (1) TELCO = die LAST-WELT: VOLLSTÄNDIG (alle Menschen, nicht nur App-User), 0–100 % AUSLASTUNG je Segment → liefert „leer"(0 %) UND „voll"(100 %) DIREKT. Das gesuchte PLAFON IST die Telco-100 %; kein Headcount, kein „erster User definiert 0" nötig. (2) COMFORT-ADJUST (1–6/12) = die PRÄFERENZ des App-Users, KEINE Last-Messung — nur WO auf 0–100 % seine Toleranz sitzt. → Sorge „keine Max-Zahlen / wo ist die Decke" löst sich (Decke = Telco-100 %); „ohne Telco nur unsere User" ist GENAU der Grund, warum Telco die Last-Quelle ist, nicht die User-Dichte (User messen nichts, sie wählen). VERSÖHNUNG mit ann_124: Telco-AUSLASTUNG (relativ zur Kapazität, vollständig, gedeckelt) ≠ Personen/m² → die nutzen wir; „relational" gilt der PRÄFERENZ-Abbildung, nicht der Messung; kein Widerspruch. Directional/bidirectional bleibt NÖTIG (Fluss-Muster ⟂ Magnitude — der Comfort-Floor betrifft nur die Magnitude). 6 (12) STUFEN = ECHT genug für Individualität (Gedränge-Wahrnehmung ist grob: ruhig/mittel/voll; Individualität = verlässliches Einlösen + relationale Abbildung, NICHT unendliche Feinheit). Offene Obergrenze = „alles recht/keine Vermeidung", braucht keine Zahl. Kunst liegt in der SCHWELLEN-Kalibrierung + Verlässlichkeit (BCK-Kernel-Arbeit, Thermostat ann_124), nicht in der Stufen-Anzahl.',
    related_panel: 'P01',
    date: '2026-06-10',
  },

  // ── SCIM3 Kosmetic-Bugs (notiert, jetzt nicht ergründen) (2026-06-10) ──

  {
    id: 'ann_121',
    category: 'next_intent',
    label: 'SCIM3 Kosmetic-Bugs — TL-Animations-Ruck · Pathworks Status-Button hängt',
    content: 'SCIM3 KOSMETIC-BUGS (Editor/SCIM3, NICHT die Target-App; notiert 2026-06-10, jetzt NICHT ergründen). (1) TL-ANIMATION RUCK/STILLSTAND: die Trygon-Loop-Animation (TrygonLoopEmblem, Leistungsblatt-Header + Usage-Manual) macht nach mehreren Minuten gelegentlich einen unnatürlichen kleinen Ruck/kurzen Stillstand — an UNTERSCHIEDLICHEN Stellen, mit unterschiedlichem Versatz/Länge, nur Sekundenbruchteile. HYPOTHESE (Claude): NICHT die Loop-Naht (die säße an FIXER Stelle jede Runde) — sondern intermittierende MAIN-THREAD-CONTENTION: GC-Pausen und/oder periodische Timer/Polls (Presence-/Anthem-/Worker-Poll, Intervalle) + React-Re-Renders, die kurz einen Animationsframe stehlen. Die Animation läuft via programmatische Keyframes/SVG auf dem MAIN-THREAD → jeder Fremd-Tick = Frame-Drop = sichtbarer Ruck; das „unterschiedliche Stellen/Längen" ist genau der Fingerabdruck externer Jank (nicht-deterministisch), nicht eines Loop-Seams. BEHEBBAR (Optionen, später): Animation compositor-only (GPU — nur transform/opacity, keine layout-/paint-/SVG-Attribut-Animation), den Emblem-Subtree von React-Re-Renders entkoppeln (memo/eigener Render), Hintergrund-Timer drosseln/koaleszieren, ggf. will-change. (2) PATHWORKS STATUS-BUTTON HÄNGT: der 1. Status-Button in Pathworks hängt manchmal kurz, wenn er aktiv ist und man z. B. auf „Registry" klickt — besonders beim ERSTEN Mal oder nach langer Zeit (nicht sicher reproduzierbar). HYPOTHESE: einmalige Kaltstart-Last — schwerer synchroner Vorgang beim ersten Klick (Lazy-Mount/erste Daten/Layout), gleiche „Main-Thread-Stall"-Familie wie (1). — NACHTRAG/Operator-Befund: der Ruck dürfte von einer REALEN Berechnung herrühren — denn die Pill-Loop scheint sich KOORDINIERT dazu kurz auszublenden (beide Animationen hitchen GLEICHZEITIG = ein gemeinsames Main-Thread-Event, NICHT zwei zufällige). Möglicherweise treten die Rucks nur in V03 auf → passt: V03 (Active-Monitor/Publishing-Monitor) pollt periodisch den Worker (Presence/CDN/QR); dieser Poll + Re-Render ist der heiße Kandidat fürs gemeinsame Stocken. → beim Ergründen ZUERST V03-Polls/Intervalle prüfen (drosseln/koaleszieren) und den Emblem-Subtree von diesen Re-Renders entkoppeln. WEITERER BELEG (Operator): beim Ruck bleibt SOGAR das Colour-Mesh in V03 stehen — obwohl es selbst KEINE Last-Berechnung o. Ä. mitträgt. Also friert in dem Moment ALLES gleichzeitig ein = bestätigt globaler Main-Thread-Block (kein Einzel-Animations-/Mesh-Problem); Verdacht V03-Poll/Re-Render erhärtet, Lokalisierung „nur in V03" gestützt. — BEIDES kosmetisch, nicht dringend; gehört zusammen, später gemeinsam ergründen.',
...

  {
    ...
    content: 'GUIDANCE/MANEGE — BACKLOG (notiert 2026-06-10, NICHT jetzt bauen; Operator-Befunde). (1) UNDO / RÜCKGÄNGIG für Routen-Edits: wenn man eine Route verkürzt (POI auslassen via Stufe 2 / tauschen via Stufe 3 / abwählen) und es SPÄT bemerkt, fehlt ein Rückgängig. Heute: einen abgewählten POI re-klicken hängt ihn ans ENDE (Reihenfolge kaputt), kein echtes Undo. → kleine Undo-Historie der selectedPoiIds-Änderungen (Schritt-zurück). (2) DEMO ↔ ECHT-WANDERN UMSCHALTEN: aktuell läuft der SIM (Sim-Walker mit SIM_LAPSE + telco-last-sim treibt die Last). Echt-Wandern = S5: navigator.geolocation.watchPosition (gleiche Schnittstelle wie der Walker → 1:1-Tausch) + reale Anthem-Last (kein telco-Slider, echte Tageszeit). OFFEN: wo/wie der Umschalter (Standort-Auto-Detect? expliziter Schalter? ?demo=1 vs. live?). Hängt an S5, Distanz-Frage. (3) STUFE-3 TAP-TO-CYCLE (mögliche Spezifikation): statt nur EINES Vorschlags per Tap auf das BLÜHENDE POI durch die (bis zu ~3) in Frage kommenden artverwandten, ruhigen POIs durchblättern; je Kandidat aktualisieren sich Name + Zeit (deltaMin) + Last-Verlauf + Icon. Voraussetzung: dompteurPick liefert Top-N statt nur Bester (shell-kit). Manege-konform (kuratierte Wenige, KEIN Routen-Shop). (4) FLOATING / GLASS-LOOK PANEL: das Confrontation/Manege-Panel verdeckt die Karte → man sieht die Routen-Änderung beim Durchblättern (3) NICHT. Ideal: floating + glass-look, damit die Route darunter sichtbar bleibt. ⚠ TRADE-OFF: Glass macht den Last-VERLAUF (Box-Hintergrund) schlecht sichtbar → einmal abwägen (Verlauf vs. Durchsicht; mögl. Kompromiss: Verlauf als schmale Leiste statt Vollfläche). VERSCHIEBBAR/Drag = nicht unmittelbar nötig. — Tap-Cycle (3) entfaltet seinen Wert erst mit dem Floating-Panel (4); sinnvoll gemeinsam als „Confrontation/Manege v2". — NACHTRÄGE (2026-06-10): (5) MANEGE 3D-VISION (für die App-Entwicklung, WENN 3D gebaut wird): die Engpass-POIs sind nervös, weil es ihnen zu voll ist — sie „schreien" wie die optimalen (poi-dompteur-energy), aber ANDERS: sie bäumen sich auf, es ist ihnen zu eng, wie Luftballons, denen die Luft ausgeht und die Aufwärts-Schub bekommen. In 2D zu schwierig → für 3D vorgemerkt. (2D-Ersatz JETZT umgesetzt: POIs verlieren hektisch+zufällig die Form — random scaleX/scaleY an wechselnden Pivots, je POI eigen, kurze Dauer.) (6) MANEGE EINZEL-TAP: die Problem-POIs in der Manege per Tap EINZELN behandeln können (nicht nur die 2 Bulk-Pole) — Aktion noch zu spezifizieren (auslassen? Tausch? = die geparkte Nachfrage-Kaskade). (7) DECISION-BUTTONS → TAP-TARGETS: die unteren/expliziten Buttons soll der User bald nach Einstieg nicht mehr brauchen; im Baufortschritt durch TOOLTIPS über abstrakten/semantischen Tap-Targets ersetzen (man tippt das Ding selbst statt einen beschrifteten Knopf). Entfernung später einfach; einstweilen behalten. Spezifizierung offen. (8) MANEGE-ANIMATION WEITER (Befund 2026-06-10): der nervöse Zufalls-Zappel sieht in klein gut aus → soll DURCHMIXT werden mit ruhigeren, aber noch schnellen Puls-Frequenzen — wie ein erschöpfter, schwer atmender Wanderer (und auch ein ausgeruhter). JE FARBKONTRAST (= Last) eine ANIMATIONS-ESKALATION: wenig belebt → ruhiger schneller Puls, sehr belebt → frantischer Zappel (Amplitude + Frequenz skalieren mit der Last). → Damit kann das VERWELKEN (poi-dompteur-energy, X/„from"-Seite) ABGELÖST werden: belebte POIs = nervös (last-eskaliert) statt welkend. (9) PRÄFERIERTES POI-ICON (Y/„to") wird NICHT blass/transparent — bleibt OPAKE, nimmt nur einen etwas stumpferen/glänzenden Farbton an (glänzende bzw. glühende Oberfläche für belebte). (Teil (9) halb-formuliert — Operator hat den Gedanken verloren; Zukunfts-Aesthetik.) → buildbar: load 0..1 je Bottleneck in die Nervös-Animation + Wilt durch Nervös ersetzen. Bezug: B2/B3 guidance_play_bauplan.md, ann_119.',
    related_panel: 'R07',
    date: '2026-06-10',
  },
...

  {
    ...
    content: 'TARGET-APP MVP-Lichtenberg — Gesamtbild (Stand 2026-06-10). Die ausgelieferte Ziel-App (sensus-core-runtime, diesenpark.com/?rep=rep-lichtenberg). Doku: docs/guidance_play_bauplan.md (aktuell, shell-kit-konform); docs/runtime_mvp.md + docs/fahrplan_mvp_lichtenberg.md (prä-shell-kit, TEILS STALE). — WAS SIE IST: Comfort-Begleiter, Slogan „Geh deinen Weg". Modell B (App routet SELBST über den Segment-Graph, keine vorgebackenen Routen), edge-lokal, Black-Box (Comfort/Route entstehen am Gerät, verlassen es nie). User stellt nur Comfort + Wanderdauer; POI short-tap=markieren, long-tap=Detail. — ARCHITEKTUR-DOKTRIN: Logik in shell-kit (in Shell-Studio testbar), Runtime = dünner Adapter. shell-kit src/app: bak.ts (solveRoute Dijkstra, routeBreachesComfort, toggleWaypoint), route.ts (renderRoute), RouteComfortBanner, ComfortSliders. — ⚠ ZWEI KARTEN-PFADE (wichtig, ann_114 vorher verwechselt): (A) ?rep=<id> → OriginPreview (src/target-app/origin/OriginPreview.tsx) = der ECHTE Auslieferungspfad, was der QR lädt + live vor Ort läuft. Hat schon Colour-Mesh + POI-Auswahl + Route (bak: solveRoute/Comfort/breach) + Comfort = Karte→POIs→Comfort→Route (Modell B). ABER NOCH KEIN geführtes Gehen/PLAY. (B) ?demo=1 → App.tsx + MapView = DEMO (privat); MapView ist @LEGACY („Löschen mit Demo-Ablösung"). Hier liegt eine separate, reichere Guidance-State-Machine (useAppMachine: proposals/BCK/guidance_active/NEXT_SEGMENT) — auf zu löschendem Code, NICHT der Zielort. — GUIDANCE/PLAY GEHÖRT IN OriginPreview (Pfad A); die bak-Route liegt dort als Polylinie bereit. — STAND BAU (2026-06-10, shell-kit v0.49.0; alles im ECHTEN Pfad OriginPreview): GUIDANCE/PLAY: S1 Walker ✓ · DurationClock ✓ (jetzt HH:MM, keine Sek, DIGIT_SCALE 0.85, Doppelpunkt slice) · S2 Play/Pause + Walker treibt Live-Position + Rest-Dauer-Uhr (EINE Steuerleiste, Reihenfolge Play→Uhr→2 Textzeilen) ✓ · S3 bewegter Positions-Marker (32×32 per bearing rotiert) ✓ · S4 nur Tour-Ende-Cue ✓ (Ankunfts-POI-Card noch OFFEN, teilt Komponente mit Long-tap) · S5 watchPosition + magnet. Kompass OFFEN (= Demo↔Echt-Switch, ann_120/121-Nachbar). SIM-Tempo SIM_LAPSE=6, WALK_SPEED 1.4 (ann_115). — BAK-DEESKALATIONSKASKADE ✓ (stille Eskalation, manifest „kein Routen-Shop"; eingefroren während der Begehung; pro Engpass einmal): Stufe 0 zeigen · Stufe 1 AUTO-Ausweichroute (solveRouteAvoiding, belebtes Netz meiden) + quantifizierter Hinweis (A1: Prozent + Mehrzeit; ab +20 % auffälliger) · Stufe 2 EINE Frage (worstBreachingLeg → [Auslassen]/[Trotzdem hin]=keptPoiIds) · Stufe 3 POI-TAUSCH-Confrontation (poi-dompteur-energy: X verwelkt ↔ Y erblüht, Funke; TAP-TO-CYCLE durch Top-N dompteurPicks via ↻i/N; Box = Last-VERLAUF jetzt→nach-Tausch; GLASS-LOOK, Route darunter sichtbar; [Tauschen]/[Bei X bleiben]). — POI-DOMPTEUR-Terminologie (ann_119): POI-Circus = Filter-Nummern kinship/energy/detour (shell-kit similarity.ts/poiDompteur.ts) · poi-dompteur-energy = Animation (PoiDompteurEnergy.tsx) · CircusPoi/DompteurPick/dompteurPick(s) · MANEGE = B3-Sammelkarte (reserviert). — UI-FARBEN/FORM: Zeit-Box = Comfort-Farbe (A3, adaptiver Kontrast) · BAK-Box = „Ohne-BAK"-Last-Farbe (A4) · Stufe-3 = Verlauf · Glass-Look auf allen Kaskaden-Boxen · Modal ≤ halbe Screenbreite (A2). POI-Custom-Ziffern auf Waypoint-Badges; Cluster-POIs einzeln wählbar (Bug-Fix). — OFFEN: B3 Manege (Sammelkarte bei Überlast — Mini-Karte spatial bottom-up + Liste top-down mit Original-Routen-Nummern + verwelkende Engpass-POIs + 2 Pole; erbt Glass-Look) · C eigene Strecken-/Kreuzungspunkt-Wahl (ann_116) · S5/Demo↔Echt-Switch · Undo für Routen-Edits (ann_120). Detail-Spez: docs/guidance_play_bauplan.md, Adjustments ann_115, Regeln ann_116/119, Backlog ann_120, Kosmetic-Bugs ann_121. — UX-ENTSCHEIDUNGEN: (a) REST-DAUER-UHR = Herzstück/erstes Anzeige-Element (zählt runter, persistent sobald Route existiert) — ERSETZT den Fortschrittsbalken-Header (raus); nächster-Halt-Info bleibt. (b) Positions-Marker = Google-Maps-GRAMMATIK (stabile Scheibe + drehendes Kite + weißer Rand) in EIGENER Akzentfarbe, KEIN Unschärfe-Kegel (Walker kennt Richtung exakt). (c) Kompass = Geräte-Sensor → S5, nicht Kern (Pfeil zeigt Richtung schon aus bearingDeg, ohne Sensor). (d) Hand-Icon = Coach-Mark/Onboarding (Long-tap lehren) + Empty-States, NICHT Live-UI/Marker. — ICONS GELIEFERT + EINSORTIERT (sensus-core-runtime/src/assets/): guidance/ = position(32x32, Kite rotierbar), play, pause, check, target-flag, compass, pointer(Hand) — Badge-Stil, feste Farben; clock/ = 0–9 + colon (4x5 monospace, beim Einbau #000→currentColor). Spec: UI-Line 24x24/stroke 1.7/currentColor; Marker 32x32 fest; Ziffern-Zelle 4x5. README in assets/guidance/. — NÄCHSTER: B3 = die MANEGE (Sammelkarte bei Überlast, erbt Glass-Look) · dann C (Streckenwahl) · S5 (echtes GPS + Demo↔Echt-Switch) · Backlog Undo (ann_120). Danach (ann_112): Auth/State-Naht/Editor-entrümpeln. ⚠ Worker ggf. wrangler deploy.',
    date: '2026-06-09',
  },
