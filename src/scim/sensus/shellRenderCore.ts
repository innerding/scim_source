// Shell Render Core — generischer, editor-/registry-/Leaflet-freier Render-Kern.
//
// EINE Quelle (Umbauplan: „eine Quelle je Engine", docs/ziel_app_umbauplan.md)
// für die generische POI-Render-Mechanik der Shell:
//   · Container-SVG          (Geometrie + Farbe → SVG-String)
//   · POI-Composite          (Container ⊕ Icon ⊕ Deco, inkl. Summit-Layout)
//   · Glyph-Reihe einer Deco (Ziffern + Einheits-Glyphen)
//   · Cluster-Mathematik     (Verschmelzung sich überlappender Entitäten)
//
// Bewusst OHNE: Vite-Glob-Registries (iconRegistry/digitGlyphs), Leaflet,
// Editor-State. Alle Asset-Inhalte werden HEREINGEREICHT (Dependency-Inversion
// über `RenderAssets`) — damit der Editor (data/-Registry) UND die Ziel-App-Shell
// (Origin-Paket) denselben Kern REFERENZIEREN statt ihn zu kopieren. Das
// editor-spezifische „Honorieren" (Icon-/Glyph-Auflösung) lebt im Adapter
// poiCatalog.composite.ts; die Runtime bringt einen eigenen Adapter über das
// Origin-Paket mit. Kein zweiter, von Hand gepflegter Render-Pfad.
...
// Quelle der rohen Glyph-SVGs ist austauschbar: Editor-Registry (Vite-Glob)
// oder Runtime-Origin-Paket. Der Kern kennt nur dieses Interface.
...
// Optionales Einheits-Glyph links, dann Ziffern, dann optionales Einheits-Glyph
// rechts. Jeder Glyph 4 viewBox-Einheiten breit, 5 hoch. Stars: Stern-Glyph
// (value)-mal wiederholt.
...
export interface CompositeInput {
  ...
  size: number;
  /** Innerer Icon-SVG-Inhalt (ohne <svg>-Hülle); '' wenn kein Icon. */
  iconInner: string;
  /** Erkannte Deco oder null (= Standard-Composite). Vom Adapter aufgelöst. */
...
// ── Cluster-Mathematik ──────────────────────────────────────────────────────
/** Eine (ggf. verschmolzene) Cluster-Entität in Pixel-Koordinaten. */
...
// Greedy: solange zwei Entitäten näher als swallowDist liegen, zu einer
// (anzahlgewichteten) Entität verschmelzen.
